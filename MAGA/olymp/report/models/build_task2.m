function build_task2()
% Simulink модель task2_model.slx — структурная схема рис.1, измерение e(t).
% Имя модели намеренно не совпадает со скриптом task2_error.m чтобы избежать
% затенения переменной воркспейса.
...
add_block('simulink/Sinks/To Workspace', [mdl '/y_out'], ...
    'VariableName','y_log','Position','[560 150 600 180]');
add_block('simulink/Sinks/To Workspace', [mdl '/e_out'], ...
    'VariableName','e_log','Position','[120 150 160 180]');
add_block('simulink/Sinks/Scope', [mdl '/Scope_e'], 'Position','[120 35 150 65]');
...
add_line(mdl, 'W4/1','y_out/1','autorouting','on');
add_line(mdl, 'Sum1/1','e_out/1','autorouting','on');
add_line(mdl, 'Sum1/1','Scope_e/1','autorouting','on');
