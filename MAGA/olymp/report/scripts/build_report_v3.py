"""Сборка v3 отчёта: OMML формулы + MATLAB-скрины Simulink моделей."""
...
def add_code(line, size=11):
    """Параграф со строкой кода (моноширинный шрифт, светло-серая подложка)."""
    p = doc.add_paragraph()
...
def add_code_block(lines, size=11):
    """Несколько строк кода с общим стилем."""
    for line in lines:
        add_code(line, size=size)

def add_matlab_block(task_num, schema_path, scope_paths, scope_labels=None, width_cm=14.5):
    """Вставить Simulink-схему и Scope-скрины из MATLAB для задания.
       schema_path  — путь к скрину схемы
       scope_paths  — список путей скринов Scope
       scope_labels — подписи под Scope-скринами (если несколько)
    """
    add_para(f'Simulink-модель задания {task_num} и результаты симуляции в MATLAB:',
             align='just', bold=True)
    add_image(schema_path, width_cm=width_cm)
    add_para(f'Рис. {task_num}.S. Simulink-схема модели task{task_num}_model.slx.',
             align='center', italic=True, size=11)
    if scope_labels is None:
        scope_labels = [''] * len(scope_paths)
    for i, (sp, lbl) in enumerate(zip(scope_paths, scope_labels)):
        add_image(sp, width_cm=width_cm)
        suffix = f' ({lbl})' if lbl else ''
        add_para(f'Рис. {task_num}.M{i+1}. Переходный процесс в Simulink Scope{suffix}.',
                 align='center', italic=True, size=11)
...
add_code_block([
    "s = tf('s');",
    "W1 = 0.7/(1.5*s+1);  W2 = 3030/(0.01*s);  W3 = 10;  W4 = 1/(0.1*s);",
    "H  = 1/(0.05*s+1);   K13 = 13;",
    "W_par     = W1 + K13;",
    "W_inner   = feedback(W2*W3, 1);",
    "W_forward = W_par * W_inner * W4;",
    "W_cl      = minreal(feedback(W_forward, H));",
])
...
add_matlab_block(
    1,
    'scrn/схема/1/1.png',
    ['scrn/схема/1/Screenshot 2026-05-20 152119.png'],
)
...
add_matlab_block(
    2,
    'scrn/схема/2/2.png',
    ['scrn/схема/2/e.png', 'scrn/схема/2/y.png'],
    scope_labels=['e(t)', 'y(t)'],
)
...
add_matlab_block(
    3,
    'scrn/схема/3/3.png',
    ['scrn/схема/3/Screenshot 2026-05-20 153004.png'],
)
...
add_matlab_block(
    4,
    'scrn/схема/4/4.png',
    ['scrn/схема/4/zn1.png', 'scrn/схема/4/zn2.png', 'scrn/схема/4/zn3.png'],
    scope_labels=['Sel=1 — кусочно-линейная',
                  'Sel=2 — полином 3-й ст.',
                  'Sel=3 — степенная'],
)
...
add_matlab_block(
    5,
    'scrn/схема/5/5.png',
    ['scrn/схема/5/y.png', 'scrn/схема/5/F.png'],
    scope_labels=['y(t) — выход системы', 'F(e) — управляющий сигнал'],
)
...
add_matlab_block(
    6,
    'scrn/схема/6/6.png',
    ['scrn/схема/6/Screenshot 2026-05-20 161158.png'],
)
...
add_matlab_block(
    7,
    'scrn/схема/7/7.png',
    ['scrn/схема/7/Screenshot 2026-05-20 160033.png'],
)
...
out = 'Отчёт_Самарин_ГУАП_3510М_v3.docx'
doc.save(out)
print('Сохранено:', out)
