...
class ReportAnalysisWindow(QWidget):
    ...
    def save_report(self):
        ...
        if file_path:
            ...
            # Apply conditional formatting to highlight sales below/above average
            df_style = df.style.apply(lambda x: ['background-color: red' if pd.to_numeric(x['Price']) < average_sales else 'background-color: green' for i in x], axis=1)
            ...
            if file_extension == "png":
                ...
                # worksheet = workbook["Product Sales"]
                # temp_plot_file = "temp_plot.png"
                # img = openpyxl.drawing.image.Image(temp_plot_file)
                # img.anchor = "A10"  # Adjust the cell coordinates as needed
