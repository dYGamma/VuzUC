...
def create_gru_model(input_shape):
    # Создание модели GRU относится к процессу построения нейронных сетей, который описан в разделе о моделях нейронных сетей в методических материалах.
    model = Sequential()
    model.add(GRU(128, input_shape=input_shape, return_sequences=True))  # Первый слой GRU с 128 нейронами
    model.add(GRU(64))  # Второй слой GRU с 64 нейронами
    model.add(Dense(1, activation='relu'))  # Полносвязный выходной слой с одной нейронной (предсказание)
    model.compile(optimizer='adam', loss='mse')  # Компиляция модели с оптимизатором Adam и функцией потерь MSE
    return model
# =============================================================


# Функция для добавления ранней остановки
# =============================================================
...
def train_models(X_full, X_filtered, y, preprocessor_full, preprocessor_filtered, scaler_full, scaler_filtered):
  ...
  # Разделение данных на обучающую и тестовую выборки (80% - обучение, 20% - тест)
    ...
    # Применение препроцессоров к данным
    ...
    X_train_filtered_standardized = scaler_filtered.fit_transform(X_train_filtered)
    X_test_filtered_standardized = scaler_filtered.transform(X_test_filtered)

    # Создание модели и ранней остановки
