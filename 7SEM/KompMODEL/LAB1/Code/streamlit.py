...
st.write(f'Уравнение Эйлера-Лагранжа: {euler_lagrange_eq}')

# Решаем уравнение Эйлера-Лагранжа
sol = dsolve(euler_lagrange_eq)
st.write(f'Общее решение: {sol}')
...
st.write(f'Найденные константы: {constants}')

# Подставляем константы в общее решение
final_solution = sol.subs(constants)
st.write(f'Оптимальное решение: {final_solution}')
