...
def evaluate_and_log(model, X_test, y_test, model_name, is_nn=False):
    """
    Оценивает модель и записывает результаты в файл.
    ...
    - is_nn: Логический флаг, указывающий, является ли модель нейронной сетью.
    """
    if is_nn:
        y_pred = model.predict(X_test)
        y_pred = y_pred.flatten()  # Преобразование предсказаний в одномерный массив
    else:
        y_pred = model.predict(X_test)
...
# Определение KerasRegressor
model = KerasRegressor(build_fn=create_model, verbose=0)

# Ограничение параметрической сетки
param_grid = {
    'batch_size': [32],  # Уменьшено количество batch_size
    'epochs': [100],     # Оставлено только одно значение для epochs
    'optimizer': ['adam']  # Оставлено только один оптимизатор
}
